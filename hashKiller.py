...
try:
    ...
    print """
Execution error:
  type 'pip install -r hashKiller.txt'.
"""
    sys.exit(-1)


# Variables
MD5DECRYPT_EMAIL = 'premium@test.com'
...
class Cracker:
    def md5(self, hash):
        print_status("Hash function: " + (Colors.PURPLE) + (Colors.BOLD) + "MD5" + (Colors.ENDC))
...
class hashkiller:
    ...
    def show_banner(self):
        print (Colors.BLUE) + (Colors.BOLD) + ("                 _                                      ")
        print "  Anyone can't give your privacy----\_______/->"
        print " That's your responsibility--------\________/->        "
        print "--------------/---------------------\_______/->                                                       "
        print "     '       /"              
        print "            /"
        print " '  V1.0  '/"
        print "          /     "
        print "_____'___/______/\/\/\/\/\/\/\/\/\//\/\/\__//" + (Colors.ENDC) + (Colors.YELLOW) + (Colors.BOLD) + ("BlackSparrow") + (Colors.ENDC)
        print "                                                 " + (Colors.RED) + (Colors.BOLD) + ("CODER-> Ajay\n") + (Colors.ENDC)

    def validate_args(self):
        parser = argparse.ArgumentParser(description="")
        parser.add_argument("-A","--option", metavar="<option>", dest="algorithm", default=None, help="Choose hash' option between %s" % ALGORITHMS)
